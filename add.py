...
def banner():
    ...
    print(f'{lg}   Version: {w}1.4{lg} | Programlayan: {w}By Sem{rs}\n')
...
print('\n' + info + lg + ' Yasaklı kullanıcı taraması yapılıyor...' + rs)
for a in accounts:
    phn = a[0]
    print(f'{plus}{grey} Kontol Ediliyor {lg}{phn}')
    ...
    for z in banned:
        accounts.remove(z)
        print(info+lg+' Banlanan hesaplar temizlendi[Hesabı Kalıcı olarak kaldırmak için manager.py modülünü kullanın.]'+rs)
    time.sleep(0.5)
    clnt.disconnect()


print(info+' Hesap Oluşturuldu!')
clr()
banner()
...
def log_status(scraped, index):
    with open('status.dat', 'wb') as f:
        ...
        f.close()
    print(f'{info}{lg} Sistem bilgileri kaydedildi {w}status.dat{lg}')
    

def exit_window():
    input(f'\n{cy} Çıkmak için ENTER tuşuna basın...')
    clr()
    banner()
    sys.exit()
...
try:
    ...
    with open('status.dat', 'rb') as f:
        ...
        f.close()
        lol = input(f'{INPUT}{cy} Önceki kullanılan gruptan üye çekmeye devam edilsin mi? {w}{status[0]}{lg}? [y/n]: {r}')
        if 'y' in lol:
            ...
            scraped_grp = input(f'{INPUT}{cy} Üyelerin çekileceği grup linkini girin: {r}')
            index = 0
except:
    scraped_grp = input(f'{INPUT}{cy} Üyelerin çekileceği grup linkini girin: {r}')
    index = 0
...
print(f'{info}{lg} Toplam Hesap Sayısı: {w}{len(accounts)}')
number_of_accs = int(input(f'{INPUT}{cy} Kaç hesap ile işlem yapılsın?: {r}'))
print(f'{info}{cy} Üyelerin Aktarılacağı grup türünü Seçin{lg}')
print(f'{cy}[0]{lg} Genel Grup')
print(f'{cy}[1]{lg} Özel Grup')
choice = int(input(f'{INPUT}{cy} Enter choice: {r}'))
if choice == 0:
    target = str(input(f'{INPUT}{cy} Genel Grubun linkini yazın (Ör:borsasancakk): {r}'))
else:
    target = str(input(f'{INPUT}{cy} Özel Grubun tam linkini yazın: {r}'))
print(f'{grey}_'*50)
#status_choice = str(input(f'{INPUT}{cy} Sadece Aktif üyeler eklensin mi?[y/n]: {r}'))
...
with open('vars.txt', 'wb') as f:
    ...
    for ab in to_use:
        pickle.dump(ab, f)
    f.close()
sleep_time = int(input(f'{INPUT}{cy} Üye ekleme arasındaki bekleme süreyi Sn olarak girin {w}[{lg}0 Hiç bekleme yapmaz{w}]: {r}'))
#print(f'{info}{lg} Joining group from {w}{number_of_accs} accounts...')
#print(f'{grey}-'*50)
print(f'{success}{lg} -- Üyeler {w}{len(to_use)}{lg} sn de bir eklenecek(s) --')
adding_status = 0
approx_members_count = 0
for acc in to_use:
    stop = index + 60
    c = TelegramClient(f'sessions/{acc[0]}', 3910389 , '86f861352f0ab76a251866059a6adbd6')
    print(f'{plus}{grey} User: {cy}{acc[0]}{lg} -- {cy}Hesap Başlatıldı... ')
    c.start(acc[0])
    ...
    try:
        if '/joinchat/' in scraped_grp:
            ...
            try:
                ...
                print(f'{plus}{grey} User: {cy}{acc_name}{lg} -- Gruplara giriş yapıldı')
            except UserAlreadyParticipantError:
                pass 
        else:
            c(JoinChannelRequest(scraped_grp))
            print(f'{plus}{grey} User: {cy}{acc_name}{lg} -- Üye çekmek için gruba girildi')
        scraped_grp_entity = c.get_entity(scraped_grp)
        if choice == 0:
            c(JoinChannelRequest(target))
            print(f'{plus}{grey} User: {cy}{acc_name}{lg} -- Üye eklemek için gruba girildi')
        ...
        print(f'{error}{r} User: {cy}{acc_name}{lg} -- Gruba girişte hata oluştu')
        print(f'{error} {r}{e}')
        continue
    print(f'{plus}{grey} User: {cy}{acc_name}{lg} -- {cy}Kaynak toplanıyor...')
    #c.get_dialogs()
    try:
        members = []
    ...
    if index >= approx_members_count:
        print(f'{error}{lg} Üye eklenmedi!')
        continue
    ...
    for user in members[index:stop]:
        ...
        if peer_flood_status == 10:
            print(f'{error}{r} Çok fazla girişim denendi. Hesaptan çıkılıyor...')
            break
        try:
            if choice == 0:
            ...
            adding_status += 1
            print(f'{info}{grey} User: {cy}{acc_name}{lg} -- Bekleme Süresi {w}{sleep_time} {lg}saniye(s)')
            time.sleep(sleep_time)
        except UserPrivacyRestrictedError:
            print(f'{minus}{grey} User: {cy}{acc_name}{lg} -- {r}Kullanıcının gizlilik ayarları kapalı')
            continue
        except PeerFloodError:
            print(f'{error}{grey} User: {cy}{acc_name}{lg} -- {r}Telegram Flood hatası.')
            peer_flood_status += 1
            continue
            ...
            print(f'{error}{grey} User: {cy}{acc_name}{lg} -- {r}Gruplardan yasaklamışsınız')
            break
        except ChatAdminRequiredError:
            print(f'{error}{grey} User: {cy}{acc_name}{lg} -- {r}Eklemek için Yöneticisi hakları gerekli')
            break
        except UserAlreadyParticipantError:
            print(f'{minus}{grey} User: {cy}{acc_name}{lg} -- {r}Kullanıcı zaten grupta')
            continue
            ...
            print(f'{error}{r} ---- Ekleme işlemi sonlandırıldı ----')
...
if adding_status != 0:
    print(f"\n{info}{lg} Ekleme oturumu sona erdi")
try:
    if index < approx_members_count:
        log_status(scraped_grp, index)
        exit_window()
